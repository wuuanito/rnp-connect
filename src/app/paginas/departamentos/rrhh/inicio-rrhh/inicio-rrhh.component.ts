...
@Component({
  ...
  styleUrl: './inicio-rrhh.component.css'
})
export class InicioRrhhComponent implements OnInit, OnDestroy {
...
fetchEventsFromApi() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/reservasRrhh`, {
      params: {
        start: startDate,
        end: endDate
      }
  ...
  handleDateSelect(selectInfo: DateSelectArg) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.http.post<any>(`${this.apiUrl}/reservasRrhh`, newEvent).subscribe(
          (response) => {
  ...
  handleEventClick(clickInfo: EventClickArg) {
    ...
    confirmDialog.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.http.delete(`${this.apiUrl}/reservasRrhh/${event.id}`).subscribe(
          () => {
            event.remove();
  ...
  private updateEventDates(event: any) {
    ...
    this.http.put(`${this.apiUrl}/reservasRrhh/${event.id}`, updatedEvent).subscribe(
      (response) => {
